...
layout: default
tags: linux unix debian ubuntu mint commandes terminal shell bash apt apt-get install remove update upgrade
licence: CC-BY-SA Ludwig Chieng
---

# Gérer les logiciels

La commande `sudo` qui permet d'exécuter une autre commande en tant qu'administrateur. Par exemple, c'est nécessaire pour exécuter `apt`. Il faudra saisir le mot de passe juste après avoir appuyé sur `Entrée ↲`. Sur Mint, en tapant le mot de passe, les caractères sont symbolisés par des « \*\*\*\*\* ». Sur Ubuntu, aucun caractère n'apparaît.
